...
def load_and_clean_file(file_path):
    ...
    for col in df.columns:
        if col.strip().lower() in {"keyword", "keywords"}:
            keyword_col = col
            break

    if not keyword_col:
        print("❌ Error: No 'Keyword' or 'Keywords' column found.")
        return None

    if keyword_col != "Keyword":
        df.rename(columns={keyword_col: "Keyword"}, inplace=True)
